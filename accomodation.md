...
<section class="accommodation-map-section" aria-labelledby="accommodation-map-title">
  ...
  <div class="stay-map-real" aria-label="Interactive accommodation map showing Munich, the U6 corridor, Garching, and selected accommodation areas">
    <div class="stay-map-art-wrap">
      <div class="stay-map-art-stage">
        <img class="stay-map-art" src="{{ '/assets/img/attractions/tarot map.1.jpeg' | relative_url }}" alt="Stylised accommodation map from Munich city centre to Garching and TUM Campus" loading="lazy">
        <button class="map-art-pin map-art-pin-venue is-map-selected" type="button" data-area="venue" aria-label="TUM Campus"></button>
        <button class="map-art-pin map-art-pin-garching" type="button" data-area="garching" aria-label="Garching"></button>
        <button class="map-art-pin map-art-pin-ismaning" type="button" data-area="ismaning" aria-label="Ismaning"></button>
        <button class="map-art-pin map-art-pin-nordfriedhof" type="button" data-area="nordfriedhof" aria-label="Nordfriedhof"></button>
        <button class="map-art-pin map-art-pin-freiheit" type="button" data-area="freiheit" aria-label="Münchner Freiheit"></button>
        <button class="map-art-pin map-art-pin-marienplatz" type="button" data-area="marienplatz" aria-label="Marienplatz"></button>
        <button class="map-art-pin map-art-pin-sendlinger" type="button" data-area="sendlinger" aria-label="Sendlinger Tor"></button>
      </div>
    </div>
    ...
    <div class="map-area-buttons" aria-label="Accommodation areas">
      ...
      <button class="map-area-bubble map-area-bubble-ismaning" type="button" data-area="ismaning" aria-controls="stay-map-panel" aria-pressed="false">
        Ismaning
      </button>
    ...
    <aside id="stay-map-panel" class="stay-map-panel" aria-live="polite">
      ...
      <div class="stay-map-panel-content">
        ...
        <a class="stay-map-panel-link" href="#tum-campus-hotels">View details</a>
      </div>
    </aside>
  </div>
</section>
...
<section id="ismaning-area" class="accommodation-section area-detail-section">
  <div class="section-heading-row">
    <div>
      <p class="accommodation-eyebrow">Suburban option</p>
      <h2>Ismaning</h2>
    </div>
    <p class="section-heading-copy">
      A quieter suburban base between Munich and Garching, useful for participants who want to stay outside the city centre while remaining within reach of the campus.
    </p>
  </div>
  ...
  <div class="area-grid">
    <article class="area-card">
      ...
      <h3>Ismaning</h3>
      <p>A calm residential option with additional accommodation possibilities and good connections toward both Munich and the Garching research campus.</p>
    </article>
  </div>
</section>
...
<section id="inner-munich-areas" class="accommodation-section area-detail-section">
  ...
  <div class="area-grid">
    ...
    <article id="freiheit-area" class="area-card">
      <span class="area-time">~25 min from venue</span>
    ...
    <article id="marienplatz-area" class="area-card">
      <span class="area-time">~30 min from venue</span>
      <h3>Marienplatz</h3>
...
.map-art-pin {
  position: absolute;
  z-index: 7;
  width: clamp(7px, .75vw, 10px);
  height: clamp(7px, .75vw, 10px);
  border: 2px solid #0f5f8e;
  border-radius: 999px;
  background: #d8eef2;
...
.map-art-pin:hover::after,
.map-art-pin:focus-visible::after,
.map-art-pin.is-map-selected::after {
  opacity: 0;
}

.map-art-pin:hover,
.map-art-pin:focus-visible,
.map-art-pin.is-map-selected {
  transform: translate(-50%, -50%) scale(1.18);
  border-color: rgba(255,255,255,.96);
  background: #fbbf24;
  box-shadow: 0 0 0 10px rgba(251,191,36,.2), 0 16px 34px rgba(15,23,42,.28);
}

.map-art-pin-venue { left: 33.4%; top: 23.43%; }
.map-art-pin-garching { left: 31%; top: 31.8%; }
.map-art-pin-ismaning { left: 53.3%; top: 39.0%; }
.map-art-pin-nordfriedhof { left: 27.25%; top: 43.4%; }
.map-art-pin-freiheit { left: 23.5%; top: 51.6%; }
.map-art-pin-marienplatz { left: 16.2%; top: 63.3%; }
.map-art-pin-sendlinger { left: 10.2%; top: 68.5%; }
...
@media (max-width:760px) {
  ...
  .map-area-buttons {
    position: absolute;
    top: calc(.85rem + ((100vw - 1.7rem) * 1190 / 1684 / 2));
    right: 1.65rem;
    z-index: 12;
    max-width: none;
    width: min(178px, 43%);
  ...
  /* Remove mobile-specific pin position overrides so desktop rules apply only */
}
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var stops = document.querySelectorAll('.map-art-pin[data-area]');
  ...
  stops.forEach(function (stop) {
    stop.addEventListener('click', function () {
      var selectedArea = stop.getAttribute('data-area');
      var matchingButton = document.querySelector('.map-area-bubble[data-area="' + selectedArea + '"]');
